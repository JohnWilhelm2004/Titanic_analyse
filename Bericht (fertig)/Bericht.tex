...
\begin{itemize}
    \item \textbf{Daten:} Zur Datenanalyse wurden der Datensatz titanic.csv und die Pakete \glqq tidyverse\grqq{} und \glqq ggplot2\grqq{} verwendet. Vor der Datenbereinigung wies die Variable \textit{Age} 177 NA-Werte (ca. 19,86 \%) auf, \textit{Embarked} besaß zwei leere Zellen (ca. 0,22 \%) und \textit{Cabin} hatte 687 leere Zellen, was einer Quote von ca. 77,10 \% entspricht.
    \item \textbf{Methodik:}
    ...
    \begin{itemize}
        ...
        \item \textbf{Embarked (Zustiegshafen):} Zwei fehlende Einträge wurden durch den Modus \glqq Southhampton\grqq{} (S) ersetzt.
...
Bei der Mehrheit der Passagiere handelte es sich um Alleinreisende (ca. 60,26 \%), wobei der Großteil von ihnen nicht überlebte (ca. 69,64 \%). Auch Familien ab einer Größe von fünf und höher sind zu 85,10 \% verstorben, wobei diese eher selten vertreten waren (ca. 5,27 \%). Bei sowohl Alleinreisenden als auch großen Familien (Größe $\ge$ 5) bestätigt ein Chi-Quadrat-Test den signifikanten Zusammenhang mit geringerer Überlebenschance($\chi^2(1) = 36.001, p < .001$, bzw. $\chi^2(1) = 10.55, p \approx .001$), wobei dieser allerdings eher schwach ist (Cramer's V von 0,20 bzw. 0,11).
